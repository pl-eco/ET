...
%{
    ...
    protected void init_keywords() {
		...
		keywords.put("adapt",		  new Integer(sym.ADAPT));
  		...
  		keywords.put("determine",      new Integer(sym.DETERMINE));
  		//keywords.put("epolicy",      new Integer(sym.EPOLICY));
  		keywords.put("where",      new Integer(sym.WHERE));
  		keywords.put("using",			new Integer(sym.USING));
    }
