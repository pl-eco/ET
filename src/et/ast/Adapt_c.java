...
public class Adapt_c extends Expr_c {
	...
	public Node typeCheck(TypeChecker tc) {
		/*
		 * the typeCheck method only create a new CSObject for Adapt
		 */
		CSNonGenericType eType = ((etTypeSystem_c) tc.typeSystem())
				.createDefaultObjectType((JL5ParsedClassType_c) ((CSNonGenericType) e
						.type()).getType());
		if (type != null && type instanceof CSBaseType) {
			ID id = ((CSBaseType) type).getID();
			if (id != null)
				eType.setID(id);
		}
		//
	...
	public Object copy() {
		Adapt_c a = (Adapt_c) super.copy();
		// if(type != null && type instanceof CSBaseType)
		// ((CSBaseType)a.type()).setID(((CSBaseType)type()).getID());
		return a;
	}
}
